...
- GNOME crash on launch: the wrapper's inline Python block imported Gdk without specifying a version; in GNOME sessions GTK4 is already loaded, so gi then tried to load Gdk 3.0 on top of 4.0 and raised ImportError. Added gi.require_version('Gdk', '3.0') before the import in both the non-root and root branches of the wrapper.
